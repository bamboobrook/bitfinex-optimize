...
## 2025-02-25 v5.2 调参：加大高执行率周期纠偏力度（部署后48h审核）

v5.1 部署后审核发现: 10-20d 周期 adj 值虽然提升，但 final 利率上调不足（adj=1.11 配合 conf_w=0.50 后被市场锚定稀释）; 60/90d adj 已到旧上限 1.35 仍不够。仅改 `predictor.py`，3项调参。

### 修改前基线（v5.1，2026-02-25 20:17 DIAG）

| 币种-周期 | exec_7d | adj | conf_w | final |
|-----------|---------|-----|--------|-------|
| fUSD 10d | 0.971 | 1.1116 | 0.50 | 6.7328 |
| fUSD 14d | 0.914 | 1.0953 | 0.65 | 7.2378 |
| fUSD 15d | 0.968 | 1.1551 | 0.51 | 4.7759 |
| fUSD 60d | 1.000 | 1.3500 | 0.65 | 9.5128 |
| fUSD 90d | 1.000 | 1.3500 | 0.65 | 14.8152 |
| fUSD 120d | 0.962 | 1.1853 | 0.54 | 8.2515 |
| fUST 60d | 1.000 | 1.3500 | 0.80 | 10.9247 |

### 修改内容（3项）

**P0: 近中周期 max_up 0.26→0.32, 长周期 max_up 0.35→0.45**
- `_get_unified_adjustment()` 4级分类参数:
  - 长周期(>=60d): max_up 0.35→**0.45**, max_down 0.30 不变
  - 中周期(20-59d): 不变 0.28/0.24
  - 近中周期(8-19d): max_up 0.26→**0.32**, max_down 0.20 不变
  - 超短周期(<8d): 不变 0.18/0.16

**P1: conf_w 极端增强门槛降低，渐进增强**
- 旧: `adj>1.15 或 <0.85` 时触发, boost上限0.15, conf_w上限0.80
- 新: `|adj-1.0|>0.08` 时触发, boost=min(abs_dev-0.08, 0.20), conf_w上限0.85
- 效果: adj=1.10 时已开始增强(+0.02), adj=1.28 时 boost=0.20(上限)

### 修改后首次预测验证（2026-02-25 20:58 DIAG）

| 币种-周期 | exec_7d | old_adj | new_adj | old_cw | new_cw | old_final | new_final | 变化 |
|-----------|---------|---------|---------|--------|--------|-----------|-----------|------|
| fUSD 10d | 0.971 | 1.1116 | 1.1631 | 0.50 | 0.58 | 6.73 | 7.71 | +14.5% |
| fUSD 14d | 0.914 | 1.0953 | 1.1429 | 0.65 | 0.71 | 7.24 | 8.06 | +11.3% |
| fUSD 15d | 0.968 | 1.1551 | 1.2084 | 0.51 | 0.63 | 4.78 | 4.78 | clipped |
| fUSD 60d | 1.000 | 1.3500 | 1.4500 | 0.65 | 0.70 | 9.51 | 10.58 | +11.3% |
| fUSD 90d | 1.000 | 1.3500 | 1.4500 | 0.65 | 0.70 | 14.82 | 15.57 | +5.1% |
| fUSD 120d | 0.962 | 1.1853 | 1.2700 | 0.54 | 0.69 | 8.25 | 9.86 | +19.5% |
| fUST 10d | 0.914 | 1.0953 | 1.1429 | 0.50 | 0.56 | 7.11 | 7.29 | +2.5% |
| fUST 14d | 0.906 | 1.0929 | 1.1400 | 0.65 | 0.71 | 11.24 | 11.46 | +2.0% |
| fUST 15d | 0.885 | 1.0704 | 1.1144 | 0.65 | 0.68 | 9.08 | 9.39 | +3.4% |
| fUST 60d | 1.000 | 1.3500 | 1.4500 | 0.80 | 0.85 | 10.92 | 12.07 | +10.5% |
| fUST 90d | 1.000 | 1.3500 | 1.4500 | 0.65 | 0.70 | 13.17 | 13.86 | +5.2% |
| fUSD 2d | 0.600 | 0.9407 | 0.9407 | 0.50 | 0.50 | 2.21 | 2.21 | 不变 |
| fUST 2d | 0.286 | 0.8400 | 0.8400 | 0.51 | 0.58 | 4.26 | 4.26 | 不变 |

纠偏方向: 高执行率组合利率全面上调(+2%~+19.5%), 低执行率组合不受影响。clipping_rate=25%。

### 审核23: 近中周期 adj 提升是否有效降低执行率（P0验证）
- **怎么查**:
  ```bash
  grep "PREDICTION_DIAG" log/ml_optimizer.log | tail -28
  sqlite3 data/lending_history.db "
  SELECT currency, period, COUNT(*), SUM(CASE WHEN status='EXECUTED' THEN 1 ELSE 0 END),
  ...
  FROM virtual_orders WHERE order_timestamp>=datetime('now','-48 hours')
    AND status IN('EXECUTED','FAILED') AND period IN(10,14,15,20) GROUP BY currency,period;"
  ```
- **期望结果**: 10-15d 执行率从 85-97% 下降趋向 70-85%（首轮预计降 5-15pp）

### 审核24: conf_w 渐进增强是否放大纠偏效果（P1验证）
- **怎么查**: DIAG 日志中 conf_w 字段
- **期望结果**: adj=1.10 的组合 conf_w 应比 base 值高 0.02; adj=1.28 时高 0.20; 上限 0.85
- **已验证**: fUST-60d conf_w=0.85(上限), fUSD-14d conf_w=0.71(从0.65+0.06)

### 审核25: 长周期 max_up 0.45 是否推动 60/90d 执行率下降（P0验证）
- **怎么查**: 60/90d final 利率对比修改前应上涨 5-11%
- **已验证**: fUSD-60d +11.3%, fUSD-90d +5.1%, fUST-60d +10.5%
- **后续**: 需要 2-3 轮(4-6h)观察执行率变化趋势
...
SELECT currency, period, COUNT(*) as total,
       ...
       ROUND(100.0*SUM(CASE WHEN status='EXECUTED' THEN 1 ELSE 0 END)/COUNT(*),1) as exec_rate
FROM virtual_orders WHERE order_timestamp>=datetime('now','-48 hours')
  AND status IN('EXECUTED','FAILED') GROUP BY currency,period ORDER BY currency,period;"
...
**目标**:
- 10-15d: 85-97% → 70-85%（首轮降5-15pp）
- 60/90d: 100% → 开始下降（利率上调 5-11%，可能需2-3轮）
- 2-7d: 保持不变或略升
- 120d fUSD: 92.9% → 下降（adj从1.19→1.27，final+19.5%）

**如果效果不足的调参方向**:
1. **10-15d 仍>85%**: 近中周期 max_up 0.32 → 0.38
